  ...
  rutinas: [
    {
      ...
      urlPdf: 'https://drive.google.com/file/d/1_m22Zju4VKNDLi2w6CvVbBklhpC9PlMO/view?usp=sharing',
      urlVer: 'https://drive.google.com/file/d/1_m22Zju4VKNDLi2w6CvVbBklhpC9PlMO/view?usp=sharing',
    },
    {
      ...
      urlPdf: 'https://drive.google.com/file/d/1gV9gtkSQT3lhLKucCv1sLipVnxk5Oh0v/view?usp=sharing',
      urlVer: 'https://drive.google.com/file/d/1gV9gtkSQT3lhLKucCv1sLipVnxk5Oh0v/view?usp=sharing',
    },
    {
      id:     'hiit',
      days:   '2 días / semana',
      ...
      goal:   'Definición',
      urlPdf: 'https://drive.google.com/file/d/1Y3v38NhIL39cG052Gt9Xrm4eniuJ90-v/view?usp=sharing',
      urlVer: 'https://drive.google.com/file/d/1Y3v38NhIL39cG052Gt9Xrm4eniuJ90-v/view?usp=sharing',
    },
    {
      id:     'gluteo',
      days:   '2 días / semana',
      ...
      goal:   'Hipertrofia',
      urlPdf: 'https://drive.google.com/file/d/1mtpY4p7los060O04Pmq2huPo0nDP7cEz/view?usp=sharing',
      urlVer: 'https://drive.google.com/file/d/1mtpY4p7los060O04Pmq2huPo0nDP7cEz/view?usp=sharing',
    },
    {
      id:     'movilidad',
...
const persist = {
  ...
  hydrate() {
    ...
    if (
      savedRutinas !== null &&
      Array.isArray(savedRutinas) &&
      savedRutinas.every(
        r => r && typeof r.name === 'string' && typeof r.id === 'string'
      )
    ) {
      state.dynamicRutinas = savedRutinas;
    }
  },
  ...
  saveRutinas() {
    persist.save(STORAGE_KEYS.RUTINAS, state.dynamicRutinas);
  },
};
...
const render = {
  ...
  rutinas() {
    ...
    const allRutinas = [...state.rutinas, ...state.dynamicRutinas];
    ...
    allRutinas.forEach(rutina => {
      ...
      const tagsLine = [rutina.level, rutina.goal].filter(Boolean).join(' · ');
      info.append(
      ...
      if (rutina.dynamic) {
        const del = utils.el('button', 'btn-pdf-delete', '✕');
        del.title = 'Eliminar rutina';
        del.addEventListener('click', () => handlers.deleteRutina(rutina.id));
  ...
  dynamicPdfs() {
    ...
    state.dynamicPdfs.forEach(entry => {
      // Compatibilidad con entradas antiguas sin id
      if (!entry.id) entry.id = 'pdf-' + Date.now() + Math.random();
      handlers._renderPdfRow(entry);
    });
  },
...
const ui = {
  ...
  switchModalTab(tab) {
    ...
    document.getElementById('mf-rutina').hidden = (tab !== 'rutina');
    ...
    document.getElementById('mt-rutina').classList.toggle('active', tab === 'rutina');
  },
  ...
  filterPdfs(category) {
    ...
    document.querySelectorAll('#pdf-library .pdf-block').forEach(block => {
      const cat = block.dataset.category || block.querySelector('.pdf-block-title')?.textContent.trim();
  ...
  /** Cierra la sesión de admin y el panel */
  adminLogout() {
    auth.logout();
    ui.closeModal();
  },
...
const handlers = {
  ...
  publishPDF() {
    ...
    const entry = { id, name, desc, cat, pages: normalizedPages, url, addedAt: Date.now() };

    // Guardar en estado y localStorage
    state.dynamicPdfs.push(entry);
    persist.savePdfs();

    // Renderizar en la biblioteca
    handlers._renderPdfRow(entry);
  ...
  _renderPdfRow(entry) {
    const { id, name, desc, cat, pages, url, addedAt } = entry;
    try {
      ...
      const isNew = typeof addedAt === 'number' && (Date.now() - addedAt) < TWO_WEEKS;
      ...
      info.innerHTML = `
        ...
        <div class="pdf-meta">${pages} páginas · ${cat}</div>
        ${desc ? `<div class="pdf-desc">${desc}</div>` : ''}
      `;
      ...
      del.className = 'btn-pdf-delete';
      del.title = 'Eliminar PDF';
      del.textContent = '✕';
      del.addEventListener('click', () => handlers.deletePDF(id, cat));

      row.append(icon, info, link, del);
  ...
  deletePDF(id, cat) {
    if (!confirm('¿Eliminar este PDF de la biblioteca?')) return;

    // Quitar del estado y localStorage
    state.dynamicPdfs = state.dynamicPdfs.filter(p => p.id !== id);
    persist.savePdfs();

    // Quitar del DOM
    const row = document.getElementById('pdf-row-' + id);
    if (row) {
      const block = row.parentElement;
      row.remove();
      // Si el bloque queda vacío (solo el título), eliminarlo también
      if (block && block.querySelectorAll('.pdf-row').length === 0) {
        block.remove();
      }
    }
  },
  ...
  /** Añade una rutina nueva desde el panel admin */
  addRutina() {
    ...
    if (!utils.notEmpty(name)) {
      alert('Introduce el nombre de la rutina.');
      return;
    }
    if (!utils.notEmpty(url) || !/^https?:\/\/.+/.test(url)) {
      alert('Introduce un enlace válido (https://...)');
    ...
    const id = 'rutina-' + Date.now();
    const entry = {
      id, name, days: days || 'A definir', desc, level, goal,
      urlPdf: url, urlVer: url, dynamic: true,
    };

    state.dynamicRutinas.push(entry);
    persist.saveRutinas();
    render.rutinas();

    document.getElementById('r-name').value  = '';
    document.getElementById('r-days').value  = '';
    document.getElementById('r-desc').value  = '';
    document.getElementById('r-goal').value  = '';
  ...
  deleteRutina(id) {
    ...
    state.dynamicRutinas = state.dynamicRutinas.filter(r => r.id !== id);
    persist.saveRutinas();
    render.rutinas();
  },

  /** Preview del enlace de rutina en tiempo real */
  previewRutinaUrl(value) {
    const preview = document.getElementById('r-url-preview');
    const text    = document.getElementById('r-url-preview-text');
    const link    = document.getElementById('r-url-preview-link');
...
function init() {
  ...
  // 7. Admin — delegación de eventos en el overlay (funciona aunque los botones se creen después)
  ...
  if (overlay) {
    overlay.addEventListener('click', e => {
      ...
      const t = e.target.closest('[id], button, a');
      if (!t) return;

      switch (t.id) {
        case 'mt-wod':           ui.switchModalTab('wod');    break;
        case 'mt-pdf':           ui.switchModalTab('pdf');    break;
        case 'mt-rutina':        ui.switchModalTab('rutina'); break;
        case 'btn-publish-wod':  WodAI.publish();             break;
        case 'btn-publish-pdf':  handlers.publishPDF();       break;
        case 'btn-add-rutina':   handlers.addRutina();        break;
        case 'btn-admin-logout': ui.adminLogout();            break;
      }

      if (t.classList.contains('modal-close') || t.classList.contains('btn-cancel-modal')) {
        ui.closeModal();
      }
    });
  }

  // 8. Admin — abrir login overlay (fuera del modal principal)
  ...
  // 10. Filtro de categorías en biblioteca PDF
...
const devTools = {
  ...
  inspect() {
    ...
    console.log('dynamicRutinas:',  structuredClone(state.dynamicRutinas));
