...
const state = {
  ...
  wod: {
    ...
    format:    '',
    diff:      '★★★☆☆',
    subtitle:  'Javier publicará el entrenamiento de este mes en breve',
    month:     '',
    note:      'Cada semana se publica un entrenamiento genérico que puedes usar si no tienes tu rutina personalizada a mano. Cualquier duda, escríbeme por WhatsApp.',
    exercises: [],
  },
...
const handlers = {
  ...
  publishPDF() {
    ...
    const entry = { id, name, cat, pages: normalizedPages, url };
    ...
    ui.closeModal();
    setTimeout(() => alert('✓ PDF "' + name + '" añadido a la biblioteca.'), 200);
  },

  _renderPdfRow(entry) {
    const { id, name, cat, pages, url } = entry;
    try {
      ...
      info.innerHTML = `<div class="pdf-name">${name}</div><div class="pdf-meta">${pages} páginas · ${cat}</div>`;
