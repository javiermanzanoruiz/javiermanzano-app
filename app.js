...
Extrae TODA la información fielmente. No inventes nada que no esté en el documento.`,
          ...
          messages: [{
            role: 'user',
            content: `Lee el contenido de este enlace de Google Drive y extrae el entrenamiento en formato JSON: ${url}

Si no puedes acceder directamente, usa el ID del archivo para construir la URL de descarga: https://drive.google.com/uc?export=download&id=${url.match(/\/d\/([a-zA-Z0-9_-]+)/)?.[1] || ''}
...
function init() {
  ...
  // 3. Splash — cargar logo desde variable JS
  const splashLogo = document.getElementById('splash-logo');
  if (splashLogo && window.__LOGO_B64__) splashLogo.src = window.__LOGO_B64__;
